...
class UserManagement extends Component
{
    ...
    // Method untuk pencarian Prodi secara real-time
    public function updatedProdiNameSearch($value)
    {
        // Atur prodi_id menjadi null saat mulai mengetik pencarian baru
        $this->resetErrorBag(['prodi_id']);

        if (strlen($value) > 1) {
            $this->prodi_results = Prodi::where('nama_prodi', 'like', '%' . $value . '%')
                ->orWhere('id', 'like', '%' . $value . '%') // Opsional: cari berdasarkan ID juga
                ->limit(10)
                ->get(['id', 'nama_prodi'])
                ->toArray();
        } else {
    ...
    public function resetInput()
    {
        $this->reset([
            ...
            'prodi_id', 'prodi_name_search', 'prodi_results', // Properti autocomplete ditambahkan
            'roleType'
        ]);
    ...
    public function saveUser()
    {
        ...
        if (!$this->prodi_id) {
             $rules['prodi_name_search'] = 'required|min:2'; 
        }

        $this->validate($rules);
        
        // ... Logika penyimpanan data tetap sama ...
    ...
    public function editUser($id)
    {
        ...
        $this->prodi_id = $user->admin->prodi_id ?? $user->dosen->prodi_id ?? $user->mahasiswa->prodi_id ?? null; // Gunakan null jika tidak ada
    ...
    public function updateUser()
    {
        $rules = [
            ...
            'prodi_id' => 'required|exists:prodis,id', // Validasi Prodi ID

        ];

        if ($this->roleType === 'dosen') {
            // Gunakan Rule::ignore(primaryKey, primaryKeyColumnName)
        ...
        if (!$this->prodi_id) {
            $rules['prodi_name_search'] = 'required|min:2'; // Paksa user untuk memilih jika ID kosong
        }

        $this->validate($rules);

        // ... Logika update data tetap sama ...
