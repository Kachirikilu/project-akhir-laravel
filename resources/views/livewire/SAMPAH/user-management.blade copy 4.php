<div class="p-6 mb-6 bg-gray-50" x-data="{ show: @entangle('showModal') }">
    ...
    <div class="bg-white shadow-lg rounded-lg overflow-hidden" id="user-results-container">
        ...
        @if ($showModal)
            {{-- Membungkus dengan Alpine.js untuk transisi dan manajemen state modal lokal --}}
            <div x-data="{ show: @entangle('showModal') }">

                {{-- Overlay Modal dan Kontainer Pusat --}}
                {{-- Z-index DITINGGALKAN di sini untuk overlay (z-50) --}}
                <div x-show="show" x-transition.opacity.duration.200ms
                    ...
                    {{-- Isi Modal Utama --}}
                    {{-- PERUBAHAN LEBAR: max-w-lg diubah menjadi max-w-4xl lg:w-4/5 (sekitar 80%) --}}
                    <div @click.outside="show = false"
                                ...
                                <div class="p-4 bg-white shadow-sm rounded-lg border border-gray-100 space-y-4">
                                    ...
                                    <div class="relative" x-data="{
                                        ...
                                        <div class="relative mt-1">
                                            <input 
                                                autocomplete="off"
                                                wire:model.live.debounce.300ms="prodi_name_search" 
                                                type="text"
                                                wire:focus="loadInitialProdiResults" 
                                        ...
                                        {{-- PERUBAHAN Z-INDEX: Tambahkan z-[60] atau z-50 untuk memastikan di atas konten modal --}}
                                        <div x-show="open && prodiResultsCount > 0" x-cloak @mousedown.prevent
                                            ...
                                            @forelse ($prodi_results as $prodi)
                                                <div wire:key="prodi-{{ $prodi['id'] }}"
                                                    ...
                                                    <span class="font-medium">{{ $prodi['nama_prodi'] }}</span>
                                                    <span class="text-xs text-gray-500 hover:text-white float-right">{{ $prodi['fakultas'] }} - ID:
                                                        {{ $prodi['id'] }}</span>
                                                </div>
                                            @empty
                        ...
                        {{-- 3. Footer/Tombol (Tetap di Bawah) --}}
                        <div
                            class="p-4 border-t flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-2 sm:space-y-0 bg-gray-50 rounded-b-lg">

                            {{-- 💡 Bagian TIPS BARU --}}
                            <div class="text-xs text-gray-500 mr-4">
                                <span class="font-semibold text-gray-700">Tips:</span>
                                <ul class="list-disc list-inside ml-2 space-y-0.5">
                                    <li class="text-sm">Leave **password field blank** to keep current password (Hanya saat
                                        Edit).</li>
                                    <li class="text-sm">Make sure all **required fields** are filled correctly.</li>
                                    <li class="text-sm">Changes will be saved immediately after submission.</li>
                                </ul>
                            </div>
